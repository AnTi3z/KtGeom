...
interface Vector2<out T : Number> : Tuple2<T> {
    operator fun <R : Number> plus(rhs: Vector2<R>): Vector2<T>
    operator fun <R : Number> minus(rhs: Vector2<R>): Vector2<T>
    operator fun unaryMinus(): Vector2<T>
    fun scaled(factor: Number): Vector2<T>
    fun <R : Number> dot(vec: Vector2<R>): T

    operator fun times(rhs: Number) = scaled(rhs)
    operator fun <R : Number> times(rhs: Vector2<R>) = dot(rhs)
...
sealed class Vector2Base<T : Number> : MutableVector2<T> {
    ...
    @Suppress("UNCHECKED_CAST")
    override fun <R : Number> dot(vec: Vector2<R>): T = when (this) {
